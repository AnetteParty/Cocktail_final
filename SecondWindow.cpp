...
SecondWindow::SecondWindow(wxWindow *parent) : wxFrame(parent, wxID_ANY, "Add Zutat", wxDefaultPosition, wxSize(900, 700)) {
    ...
    buttonChange->Connect(wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(SecondWindow::OnButtonChangeClicked), NULL, this);
...
void SecondWindow::OnButtonChangeClicked(wxCommandEvent& event) {

    if (listBox->GetSelection() != wxNOT_FOUND) {
        wxString oldName = listBox->GetStringSelection();
        wxString newName = textCtrlName->GetValue();
        ...
        if (!newName.IsEmpty()) {
            ...
            wxString query = wxString::Format("UPDATE zutaten SET name = '%s' WHERE name = '%s'", newName, oldName);
            ...
            rc = sqlite3_step(stmt);
            if (rc != SQLITE_DONE) {
                std::cerr << "Fehler bei sqlite3_step: " << sqlite3_errmsg(db) << std::endl;
                return;
            }
